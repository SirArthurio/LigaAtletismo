...
import { ObtenerEntrenadores } from "../../API/Data.jsx";
...
const entrenadores = [{ nombre: "luis enrique", id: 363636 }];
...
const DeportistasCrud = () => {
  ...
  const handleFileChange = (e) => {
    ...
    setPreview(file ? URL.createObjectURL(file) : null); // Opcional: Mostrar vista previa
  };
  ...
  const agregarDeportista = async () => {
    try {
      ...
      formData.append(
        "birthdate",
        convertirFechaAFormatoDate(deportistaEditado.birthdate)
      );
          ...
          return (
            <div className="relative flex items-center gap-2">
              ...
              <Tooltip color="danger" content="Delete user">
                <button
                  onClick={async () => await eliminarDeportista(item.document)}
                  className="text-lg text-danger cursor-pointer active:opacity-50"
                >
                  <DeleteIcon />
                </button>
  ...
  return (
    <div className="m-3 p-5 border-2 rounded-md">
        ...
        <form className="grid grid-cols-2 p-2">
          ...
          <div className="flex space-x-4">
            ...
            >
              {tipoDeDocumento.map((tipoDeDocumento, index) => (
                <SelectItem
                  key={tipoDeDocumento.nombre}
                  value={tipoDeDocumento.nombre}
                >
          ...
          <input
            ...
            onChange={(e) => {
              const file = e.target.files[0];
              setDeportistaEditado((prev) => ({ ...prev, img: file }));
              setPreview(file ? URL.createObjectURL(file) : null); // Opcional, para mostrar la vista previa
            }}
          />
          ...
          >
            {coach.map((coach, index) => (
              <SelectItem key={coach.document} value={coach.document}>
                {coach.name}
              </SelectItem>
            ))}
          </Select>
      ...
      <section className="m-3 p-5 border-2 rounded-md">
        <div className="p-4">{TablaDeportista()}</div>
      </section>
    </div>
  );
};
